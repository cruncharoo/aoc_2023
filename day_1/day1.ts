...
function find_int(data: string, start: "first" | "last") {
  let i: number;
  start == "first" ? (i = 0) : (i = data.length - 1);
  while (nums.includes(data[i]) == false) {
    start == "first" ? (i += 1) : (i -= 1);
  }
  const first_num = data[i];
  const first_num_int = nums.indexOf(first_num);
  return first_num_int;
}

function part_one_solve(data: Array<string>) {
  let calibration_values: number[] = [];
  data.forEach((element: string) => {
    let calibration_value = 0;
    calibration_value += find_int(element, "first") * 10;
    calibration_value += find_int(element, "last");
    calibration_values.push(calibration_value);
  });
  const sum_of_calibration_values = calibration_values.reduce((acc, cv) => {
    return acc + cv;
  }, 0);
  return sum_of_calibration_values;
}

function find_digits(data: string) {
  let i: number = 0;
  let digit_chars: number = 0;
  let found = false;
  while (digit_chars == 0 && i < data.length && !found) {
    if (nums.includes(data[i])) {
      return nums.indexOf(data[i]);
    }
    [3, 4, 5].forEach((int) => {
      if (digits.includes(data.slice(i, i + int)) && !found) {
        digit_chars = int;
        found = true;
      }
    });
  ...
  if (digit_chars != 0) {
    let found_word = data.slice(i, i + digit_chars);
    return digits.indexOf(found_word);
  }
  return 0;
}

function find_digits_back(data: string) {
  let i: number = data.length - 1;
  let digit_chars: number = 0;
  let found = false;
  while (digit_chars == 0 && i >= 0 && !found) {
    if (nums.includes(data[i])) {
      return nums.indexOf(data[i]);
    } else {
      [3, 4, 5].forEach((int) => {
        if (digits.includes(data.slice(i - int + 1, i + 1)) && !found) {
          digit_chars = int;
          found = true;
        }
      });
  ...
  if (digit_chars != 0) {
    let found_word = data.slice(i - digit_chars + 1, i + 1);
    return digits.indexOf(found_word);
  }
  return 0;
}

function part_two_solve(data: Array<string>) {
  let calibration_values: number[] = [];
  data.forEach((element: string) => {
    let calibration_value = 0;
    calibration_value += find_digits(element) * 10;
    calibration_value += find_digits_back(element);
    calibration_values.push(calibration_value);
  });
  const sum_of_calibration_values = calibration_values.reduce((acc, cv) => {
    return acc + cv;
...
console.log(part_one_solve(real_data));
console.log(part_two_solve(real_data));
