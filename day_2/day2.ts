...
function day_one_solve(
  ...
  data.forEach((game) => {
    game_count += 1;
    let maxes = max_color_per_game(game_formatter(game));
    if (
      maxes.red <= max_red &&
      maxes.green <= max_green &&
      maxes.blue <= max_blue
    ) {
...
function day_two_solve(data: Array<string>) {
  ...
  data.forEach((game) => {
    let max = 1;
    let colors = max_color_per_game(game_formatter(game));
    max = max * colors.red;
    max = max * colors.green;
    max = max * colors.blue;
    acc += max;
  });
  return acc;
